<template>
  <div class="relative flex gap-3 p-4 bg-gry-100 hover:bg-gry-200 border border-gry-300 rounded-lg cursor-pointer"
    @click="emit('viewDetail', recruit.recruitId)">
    ...
    <div class="space-y-4 w-full flex flex-col">
      ...
      <div class="space-y-1 font-caption text-gry-900">
        ...
        <div class="grid grid-cols-2">
          <p>고용형태 : {{ recruit.workType }}</p>
          <p>경력 : {{ recruit.careerType }}</p>
        </div>
        <p>근무지 : {{ recruit.workPlace }}</p>
      </div>
      ...
      <div class="grid grid-cols-2 font-h4">
        <div class="flex items-center space-x-2">
          ...
          <span class="text-gry-900">{{ recruit.registerDate }}</span>
        </div>
        <div class="flex items-center space-x-2">
          <span class="text-gry-700">마감일</span>
          <span class="text-gry-900">{{ recruit.endDate }}</span>
        </div>
      </div>
      ...
      <slot>
        <div class="flex space-x-2">
          <CircleButton @click="emit('addFavorite', recruit.recruitId)" />
          <ToggleButton @click="emit('addChallenge', recruit.recruitId)">
            <template #icon>
              <Icon fill="point">
                <LinePlusIcon />
              </Icon>
