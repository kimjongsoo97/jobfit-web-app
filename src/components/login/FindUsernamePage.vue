<template>
  <div class="max-w-[480px] w-full">
    ...
    <div class="h-[2px] bg-gry-300 my-4"></div>
    <Button variant="gray" size="lg" @click="handleFindPasswordPage">
      <template #text>
        비밀번호 찾기
      </template>
    </Button>
  ...
  <SubmitModal v-if="isSubmitModal" @handleFindPasswordPage="handleFindPasswordPage" @handleLoginPage="handleLoginPage"
    :email="user.email" />
</template>
...
const handleSubmit = async () => {
  try {
    ...
    await LoginApi.findUsername(payload)
    isSubmitModal.value = true
  } catch (err: any) {
    console.error('아이디 찾기 실패:', err.response?.data || err.message)
  }
...
const otpCreate = async () => {
  ...
  try {
    await LoginApi.otpCreate(user.email, "username");
